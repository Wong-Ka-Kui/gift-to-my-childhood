...
import { scheduledArea, requestSeat, stepLife, standUp, freePoint, worldX, type PetLife, type LifeWorld, type Seat } from './pet-life';
function fixture() {
...
describe('daily pet routines',()=>{
  ...
  it('walks visibly around the campus both ways with no position jumps',()=>{
    const {pet,world}=fixture();
    let sawRoad = false;
    const walk = (now: number, target: 'bedroom' | 'classroom') => {
      for(let i=0;i<4000;i++) {
        ...
        if(pet.wander.z>6) sawRoad = true;
        if(pet.area===target && !pet.destination) break;
      }
    };
    walk(120000,'classroom');
