...
function fixture(){
 const seats:Seat[]=Array.from({length:4},(_,i)=>({id:`chair${i}`,area:'classroom',x:i*2-3,z:2,y:.76,yaw:Math.PI,width:1,depth:1}));
 ...
 const run=(minute:number)=>{const now=(minute<480?minute+1440:minute)-480;for(let i=0;i<100;i++)for(const p of pets.values())stepLife(world,p,.05,false,now*1000);};
...
describe('four-person required schedule',()=>{
 ...
 it('seats four stopped pets in unique chairs for every lesson and releases them at breaks',()=>{
  const f=fixture();for(const m of [600,810,1080]){f.run(m);expect(new Set([...f.pets.values()].map(p=>p.seat?.id)).size).toBe(4);for(const p of f.pets.values()){expect(p.area).toBe('classroom');expect(p.sitBlend).toBe(1);expect(standUp(f.world,p)).toBe(false);}f.run(m===600?720:m===810?1020:1140);for(const p of f.pets.values())expect(p.seat).toBeNull();}
 });
 ...
 it('reserves upper and lower beds independently for manual sleep',()=>{const f=fixture(),[a,b]=[...f.pets.values()];expect(requestBed(f.world,a,f.beds[0])).toBe(true);expect(requestBed(f.world,b,f.beds[0])).toBe(false);expect(requestBed(f.world,b,f.beds[1])).toBe(true);f.run(480);expect(a.sleepBlend).toBe(1);expect(b.sleepBlend).toBe(1);});
});
