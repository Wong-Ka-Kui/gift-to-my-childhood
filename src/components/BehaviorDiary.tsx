...
import { localDateKey, isFutureDate } from "../lib/diary-date";
...
export default function BehaviorDiary({ pets, onClose }: { pets: readonly PetRecord[]; onClose: () => void }) {
  ...
  useEffect(()=>{const update=()=>setToday(new Date());const timer=setInterval(update,30000);window.addEventListener("focus",update);return()=>{clearInterval(timer);window.removeEventListener("focus",update);};},[]);
  ...
  const entries = useMemo(() => {
    ...
    for (let day = 1; day <= count; day += 1) pets.forEach((pet, index) => {
      if (isFutureDate(dateKey(view.year,view.month,day),today)) return;
  ...
  const moveMonth = (delta: number) => { const next = new Date(view.year, view.month + delta, 1); if (isFutureDate(localDateKey(next),today)) return; setView({ year: next.getFullYear(), month: next.getMonth() }); setSelectedDate(dateKey(next.getFullYear(), next.getMonth(), 1)); };
  return <aside className="diary-drawer" aria-label="宠物行为日记">
    ...
    <div className="diary-month-nav"><button type="button" onClick={() => moveMonth(-1)} aria-label="上个月">‹</button><strong>{monthLabel(view.year, view.month)}</strong><button type="button" disabled={view.year === today.getFullYear() && view.month === today.getMonth()} onClick={() => moveMonth(1)} aria-label="下个月">›</button></div>
    <div className="diary-weekdays">{["日", "一", "二", "三", "四", "五", "六"].map((day) => <span key={day}>{day}</span>)}</div>
    <div className="diary-calendar">{Array.from({ length: monthStart }, (_, index) => <span key={`empty-${index}`} className="diary-day is-empty" />)}{Array.from({ length: monthDays }, (_, index) => { const day = index + 1; const key = dateKey(view.year, view.month, day); const hasEntries = entriesByDate.has(key); return <button key={key} type="button" disabled={key>todayKey} className={`diary-day${selectedDate === key ? " is-selected" : ""}${hasEntries ? " has-memory" : ""}`} onClick={() => setSelectedDate(key)}><span>{day}</span>{hasEntries ? <i aria-label="有行为记录" /> : null}</button>; })}</div>
